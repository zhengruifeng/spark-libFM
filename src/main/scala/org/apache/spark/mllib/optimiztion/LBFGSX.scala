package org.apache.spark.mllib.optimiztion

import breeze.linalg.DenseVector
import breeze.optimize.{DiffFunction, CachedDiffFunction}
...
import org.apache.spark.mllib.optimization.{LBFGS, Optimizer, Updater, Gradient}
import org.apache.spark.mllib.optimiztion.GradientDescentX._
import org.apache.spark.rdd.RDD

import scala.collection.mutable.ArrayBuffer

/**
 * Created by zrf on 4/23/15.
 */
class LBFGSX(private var gradient: Gradient, private var updater: Updater)
...
object LBFGSX extends Logging {
  ...
  def runLBFGS(
    ...
    while (states.hasNext) {
      logWarning(s"Iteration ${state.iter}, LOSS : ${state.value}")
    ...
    logInfo("LBFGS.runLBFGS finished. Total losses %s".format(
      lossHistory.mkString(", ")))
  ...
  private class CostFun(
    ...
    override def calculate(weights: BDV[Double]): (Double, BDV[Double]) = {
      ...
      logWarning(s"Iteration X, LOSS: ${lossSum / numExamples}, REG: $regVal")
